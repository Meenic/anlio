...
mock.module('@/modules/sse/broadcaster', () => ({
...
mock.module('@/modules/sse/offline-removal', () => ({
  scheduleOfflineRemoval: () => {},
  cancelOfflineRemovalTimer: () => {},
}));

mock.module('@/modules/sse/registry', () => ({
  registry: new Map(),
  unregisterClient: () => {},
}));

mock.module('@/modules/game/engine', () => ({
  startGame: async () => {},
...
let roomStore: typeof import('@/modules/room/store');
...
beforeAll(async () => {
  ...
  roomStore = await import('@/modules/room/store');
});
